...
import medicamentRequestService from "../../../Services/MedicamentRequestService";

const RequestListPatient = (props) => {
    ...
    const retrieveDoctors = () => {
        ...
        medicamentRequestService.getAll(params)
            .then((response) => {
    ...
    const removeAllDoctors = () => {
        medicamentRequestService.removeAll()
            .then((response) => {
                console.log(response.data);
                refreshList();
            })
    ...
    const openDoctor = (rowIndex) => {
        ...
        medicamentRequestService.get(id);
    };

    const deleteDoctor = (rowIndex) => {
        const id = doctorsRef.current[rowIndex].id;
        medicamentRequestService.remove(id)
            .then((response) => {
    ...
    useEffect(() => {
        medicamentRequestService.getAll()
            .then(response => {
    ...
    return (
        <MainContainer>
            <div className="mb-3" style={{ color: "white"}}>
                <HeaderText className='GradientText' style={{ fontSize: "2.6em", fontWeight: "bolder"}}>Gyógyszerigénylések</HeaderText>
            </div>
            <div>
                {/*<Table2 />*/}
            </div>
